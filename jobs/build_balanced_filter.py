...
the 14 val14 categories. Writes, into critic-training-data/:
  - critic_train_balanced.yaml         (a nuPlan ScenarioFilter with the explicit
                                        token list -> fast, reproducible collection)
  - critic_train_balanced_token2db.json (token -> db basename, for fast per-worker build)
  - critic_train_balanced_summary.json  (per-category counts + disjointness check)
...
def main():
    ...
    fy = os.path.join(CTD, "critic_train_balanced.yaml")
    with open(fy, "w") as f:
    ...
    json.dump(token2db, open(os.path.join(CTD, "critic_train_balanced_token2db.json"), "w"), indent=2)
    summary = {
    ...
    json.dump(summary, open(os.path.join(CTD, "critic_train_balanced_summary.json"), "w"), indent=2)
